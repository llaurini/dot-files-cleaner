...
class TestPackageCheckerLoad:
    ...
    def test_load_graceful_on_timeout(self) -> None:
        ...
        import subprocess
        with patch("subprocess.run", side_effect=subprocess.TimeoutExpired(cmd="dpkg-query", timeout=30)):
            checker = PackageChecker()
            checker.load()
...
class TestIsInstalledDpkg:
    ...
    def test_returns_true_for_installed_package(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
        checker = checker_with_packages(["vim", "git"])
        assert checker.is_installed_dpkg("vim") is True

    def test_returns_false_for_missing_package(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
        checker = checker_with_packages(["vim"])
        assert checker.is_installed_dpkg("zoom") is False

    def test_case_insensitive(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
        """La verifica è case-insensitive."""
        checker = checker_with_packages(["vim"])
        assert checker.is_installed_dpkg("VIM") is True
        assert checker.is_installed_dpkg("Vim") is True

    def test_empty_packages_returns_false(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
...
class TestIsBinaryAvailable:
    ...
    def test_returns_true_for_existing_binary(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
    ...
    def test_returns_false_for_missing_binary(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
...
class TestCheckPackages:
    ...
    def test_splits_installed_and_uninstalled(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
        checker = checker_with_packages(["vim", "git"])
        installed, uninstalled = checker.check_packages(["vim", "git", "zoom", "nonexistent"])
        assert "vim" in installed
        assert "git" in installed
        assert "zoom" in uninstalled
        assert "nonexistent" in uninstalled

    def test_all_installed(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
        checker = checker_with_packages(["vim", "git"])
        installed, uninstalled = checker.check_packages(["vim", "git"])
        assert installed == ["vim", "git"]
        assert uninstalled == []

    def test_all_uninstalled(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
        checker = checker_with_packages(["vim"])
    ...
    def test_empty_input_returns_empty_lists(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
        checker = checker_with_packages(["vim"])
        installed, uninstalled = checker.check_packages([])
        assert installed == []
        assert uninstalled == []

    def test_binary_fallback_marks_as_installed(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
...
class TestFindMatchingPackages:
    ...
    def test_exact_name_matches(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
        checker = checker_with_packages(["vim", "git", "python3"])
        result = checker.find_matching_packages("vim")
        assert "vim" in result

    def test_name_as_first_token_matches(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
    ...
    def test_no_false_positive_micro_in_microsoft(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
        """
        'micro' NON deve trovare 'microsoft-edge-beta'.
        Questo è l'invariante critico anti-falsi-positivi.
        """
        checker = checker_with_packages(["microsoft-edge-beta", "microsoft-edge-stable"])
    ...
    def test_no_false_positive_git_in_gitk(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
    ...
    def test_returns_sorted_list(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
        checker = checker_with_packages(["zsh-syntax-highlighting", "zsh-autosuggestions", "zsh"])
        result = checker.find_matching_packages("zsh")
        assert result == sorted(result)

    def test_empty_packages_returns_empty(self, checker_with_packages: Callable[[list[str]], PackageChecker]) -> None:
        checker = checker_with_packages([])
        result = checker.find_matching_packages("vim")
        assert result == []
